...
def load_dataset(name="dino"):
    if name == "dino":
        df = pd.read_csv(f"{current_path}/seed_datasets/Datasaurus_data.csv", header=None, names=['x','y'])
    elif name == "rando":
        df = pd.read_csv(f"{current_path}/seed_datasets/random_cloud.csv")
        df = df[['x', 'y']]
    elif name == "slant":
    	df = pd.read_csv(f"{current_path}/seed_datasets/slanted_less.csv")
    	df = df[['x', 'y']]
    elif name == "big_slant":
        df = pd.read_csv(f"{current_path}/seed_datasets/less_angled_blob.csv")
        df = df[['x', 'y']]
        df = df.clip(1, 99)
...
#
def run_pattern(df, target, directory, iters = 100000, num_frames=100, decimals=2, shake=0.2,
    ...
    for i in looper(iters+1, leave=True, ascii=True, desc=target + " pattern"):
        ...
        for x in range(write_frames.count(i)):
            save_scatter_and_results(r_good, target + "-image-"+format(int(frame_count), '05'), dp=150, labels = labels, directory=directory)
...
def do_single_run(start_dataset, target, iterations=100000, decimals=2, num_frames=100):
    ...
    temp = run_pattern(df, target, iters=iterations, num_frames=num_frames, directory=f'{start_dataset}_{target}')
    return temp
...
def create_gifs(shape_start: str, shape_end: str):

    # Create the frames
    frames = []
    imgs = glob.glob(f"{shape_start}_{shape_end}/*.png")
    for i in imgs:
    ...
    frames[0].save(f"gifs/{shape_start}_{shape_end}.gif", format='GIF',
...
def main(args):
    ...
    if args.iters:
        it = int(args.iters)
    if args.decimals:
        de = int(args.decimals)
    if args.decimals:
        frames = int(args.frames)


    shape_start = args.shape_start
    shape_end = args.shape_end
    save_directory = f'{shape_start}_{shape_end}'
